...
def insert_rep_fraud(conn):
    conn.cursor().execute('''
        with tmp as (    
            ...
            left join V_TERMINALS t2 on t1.terminal = t2.terminal_id
            ...
            select distinct
                ...
                case
                    ...
                    when terminal_city != lag(terminal_city) over (partition by card_num order by trans_date) 
                        ...
                        strftime('%s', lag(trans_date) over (partition by card_num order by trans_date)) / 60 <= 60
                    ...
                    when 
                        ...
                        and amt - lag(amt, 1) over (partition by card_num order by trans_date) < 0
                        and lag(amt, 1) over (partition by card_num order by trans_date) -
                        lag(amt, 2) over (partition by card_num order by trans_date) < 0
                        and strftime('%s', trans_date) / 60 -
                        ...
                        and oper_type in ('PAYMENT', 'WITHDRAW')
                        then 'select_amount'
                end as event_type
            from tmp
            )
